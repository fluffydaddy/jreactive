...
import io.fluffydaddy.jutils.annotation.Nullable;
 ...
 * <p>
 * When subscribing to a data source, this builder allows to configure:
 * <ul>
    ...
    class ActionObserver implements DataObserver<T>, DelegatingObserver<T> {
        ...
        private void callOnError(Throwable th, String msgNoErrorObserver) {
            if (errorObserver != null) {
                if (!subscription.isCanceled()) {
                    if (scheduler != null) {
                        scheduler.schedule(schedulerRunOnError, th);
                    } else {
        ...
        void callOnData(final T data) {
            if (!subscription.isCanceled()) {
                if (scheduler != null) {
                    scheduler.schedule(schedulerRunOnData, data);
                } else {
                    try {
        ...
        class SchedulerRunOnChange implements Reactive<T> {
            @Override
        ...
        class SchedulerRunOnError implements Reactive<Throwable> {
            @Override
